...
from .views import (
    MedidaListView, MedidaDetailView, MedidaDeleteView, medida_form_view
)
...
urlpatterns = [ #pylint: disable=invalid-name
    ...
    re_path(r'^editar/(?P<pk>\d+)/$', medida_form_view,
            name='editar-medida'),
    re_path(r'^(?P<pk>\d+)/$', MedidaDetailView.as_view(),
            name='detalles-medida'),
    re_path(r'^borrar/(?P<pk>\d+)/$', MedidaDeleteView.as_view(),
            name='borrar-medida'),
]
